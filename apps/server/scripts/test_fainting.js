// B-US-07 Verification: Fainting & Pokemon Swap
...
socket1.on("lobby_status", (data) => {
    ...
    if (p1Data.team && p1Data.team.length === 3 && !p2Created) {
        ...
        socket1.on("turn_result", (data) => {
            turnCount++;
            console.log(`Turn ${turnCount}: damage=${data.damage}, hp=${data.remainingHp}, defeated=${data.isDefeated}`);
            ...
            if (data.pokemonFainted) {
                ...
                console.log("TEST SUCCESSFUL — B-US-07 VERIFIED");
                process.exit(0);
            } else {
                setTimeout(() => {
                    socket1.emit("attack");
...
setTimeout(() => {
    console.log("TEST TIMED OUT after 40s");
    process.exit(1);
}, 40000);
